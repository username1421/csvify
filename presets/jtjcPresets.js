...
window.presets = [
  {
    ...
    description:
      "Удаляет события с датами в заданном диапазоне и выгружает удалённые события в CSV. Событие удаляется, если оно не повторяется, дата начала события больше заданной даты начала диапазона, а дата конца события меньше заданной даты конца диапазона. Необходимый диапазон для удаления задаётся в константе DATES_OF_EVENTS_TO_DELETE.",
